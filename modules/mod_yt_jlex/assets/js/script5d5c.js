...
function YTF(s) {
	...
	this.formatTime=(function(duration){
		...
		var hours = typeof parts[1]!='undefined' ? parseInt(parts[1]):0;
	    var minutes = typeof parts[2]!='undefined' ? parseInt(parts[2]):0;
	    var seconds = typeof parts[3]!='undefined' ? parseInt(parts[3]):0;
	...
	this.str2Time=(function(timestring,format){
	...
	this.shortStr=(function(str,maxword){
	...
	this.bindHTML=(function(callback){
		/* Bind size */
		var findtpl=function(){
			...
			if (s.show_list) {
				if (w < s.small_devices) {
					...
					if (s.style=='ver') {
						subOfWidth = 114;
						$p.find('.ytf-sidebar').height((w-114)*(s.ratio=='4:3'?3/4:9/16)-35);
					}	
				} else {
					activeSize = 'ytf-lg';
					if (s.style=='ver') {
						subOfWidth = 300;
						$p.find('.ytf-sidebar').height((w-300)*(s.ratio=='4:3'?3/4:9/16));
					}			
				}
			...
			if (s.style=='grid' && s.listcenter) {
				if (activeSize=='ytf-md' || activeSize=='ytf-lg') {
					...
					$p.find('.swiper-wrapper,.ytfBlock').removeAttr('style');
				}
			}
		};
		
		$j(window).on('resize',function(){findtpl();});
		if (s.popup) {
	...
	this.addVid2List=function() {
		if (!vidData.length) return;
		var html='',offset=$p.find('.swiper-wrapper .ytfBlock').length?$p.find('.swiper-wrapper .ytfBlock:last').attr('data-index')*1:0;
		$j.each(vidData,function(k,v){
			k+=offset;
			var thumb = 'http://i1.ytimg.com/vi/' + v.id +'/'+s.thumbsize;
		...
		vidData.splice(0,vidData.length);
	...
	this.scrollbar=(function(params){
		var oc=this,
			$ob=$j(params.ob),
	...
	this.getData = (function(){
		...
		getEntryNext();
		_this.getDetailVides();
		// Create intro if enable
		if (s.intromode&&s.bgurl) {
			var bgurl = !/^http/.test(s.bgurl) ? s.root+s.bgurl : s.bgurl;
			var html='<div class="ytf-bglist" style="background-image:url('+bgurl+')">'+(s.introtext?s.introtext:'<span class="ytf-playbutton" onclick="ytf_'+s.moduleid+'.play(this)"></span>')+'</div>';
	...
	this.events=function(){
		...
		$p.on('click','.ytfBlock',function(e){
			if ($j(this).hasClass('active') || $j(e.target).is('i') ) return false;
			var id = $j(this).attr('data-index')*1;
			if (typeof ytdata[id] != 'undefined') {
				$p.find('.ytfBlock[data-index='+id+']').addClass('active').siblings().removeClass('active');
				player.loadVideoById(ytdata[id].id);
				vidCurrent=id;
		...
		.on('click','._desc',function(e){
			...
			var id = $j(this).attr('data-index'),
				content=ytdata[id].description;
			popup.open( content.replace(/\r\n|\r|\n/g,"<br />"),'html',$j(this) );
		});
	},

	this.getDetailVides = (function(){
		var run=window.setInterval(function(){
			if (ytdataid.length) {
				...
				var findDetail=function(){
					...
					var listId=[],i3=1;
					$j.each(ytdataid,function(k,v){
						listId.push(v);
						if (i3>=16) return false; // Stop query
						i3++;
					});
					ytdataid.splice(0, i3);
					$j.getJSON('https://www.googleapis.com/youtube/v3/videos?part=snippet,contentDetails',{id:listId.join(','),key:s.key},function(d){
						if (d.items.length) {
							$j.each(d.items,function(k,v){
								var itemData={
									id:v.id,
								...
								ytdata.push(itemData);
								vidData.push(itemData);
								if (ytdata.length==1) {
									_this.player();
								}
							});
	...
	this.onPlayerReady=function(){
		...
		if (isMobile.any() && s.autoplay) {
			player.loadVideoById(ytdata[0].id);
			vidCurrent=0;
			player.setPlaybackQuality(s.quality);
		}
	},
	...
	this.onPlayerStateChange=function(e){
		if (e.data==YT.PlayerState.ENDED && s.autonext) {
			if (s.random) {
				vidCurrent=Math.floor(Math.random() * ytdata.length);
				player.loadVideoById(ytdata[vidCurrent].id);
			} else {
				if (typeof ytdata[vidCurrent+1] != 'undefined'){
					player.loadVideoById(ytdata[vidCurrent+1].id);
					vidCurrent+=1;			
				} else {
					vidCurrent=0;
					player.loadVideoById(ytdata[0].id);
